...
ratings %>% ggplot(aes(x=ratingq)) + geom_histogram();
```
...
ratings %>% 
  ...
  arrange(rank) %>% 
  mutate(cs=cumsum(popularity)) %>% View
...
ratings %>% group_by(user_id) %>% summarize(count=n()) %>% ggplot(aes(x=count)) + geom_histogram()
```
