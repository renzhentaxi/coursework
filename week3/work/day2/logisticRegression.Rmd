...
process <- function(location)
{
  ...
  data <- data %>% mutate(age.bin = addNA(cut(Age,q)), 
                              Age = ifelse(is.na(Age),median(Age,na.rm=T),Age))
  levels(data$age.bin) <- c("0-25%","25-50%","50-75%","75-100%","Missing")
  return (data)
}

test <- process("test.csv");
train <- process("titanic_train.csv")
...
model <- glm(formula = Survived~Sex*poly(Age,2) + Parch + Pclass + poly(SibSp,3), data = train, family = "binomial")
test$Survived <- ifelse(predict(model,test,type="response")>.5,1,0)
test <- test %>% select(PassengerId,Survived)
write.csv(test,file = "test_result.csv",quote=F, col.names = F, row.names = F)
testModel(Survived~Sex*poly(Age,2) + Parch + Pclass + poly(SibSp,3), train, train)
